...
int main() {
    ...
    for (const auto  payload : payloads) {
       ...
       // std::cout <<"Enter the value of " <<get<0>(param) << "\n";
        std::string para="";
            if(cnt==1){
                url+=get<0>(param)+"="+payload;
                cnt++;
            }
            else if(std::get<1>(param)=="submit"&&std::get<0>(param).length()!=0&&std::get<0>(param).length()!=0){
                url+="&"+get<0>(param)+"="+get<2>(param);
            }
            else{
                url+="&"+get<0>(param)+"="+para;
                cnt++;
            }
        }
        std::cout << url << "\n";
